  ...
  return Effect.gen(function* () {
    ...
    try {
      ...
      return yield* Effect.fail(
        new DataLoadError(path, e instanceof Error ? e.message : "Unknown error"),
      );
    }
  });
}
...
function mergeProperties(
  csstreeData: CsstreeData,
  vscodeData: VSCodeCSSData,
): MergedProperty[] {
...
function extractCssWideKeywords(csstreeData: CsstreeData): string[] {
  return csstreeData.cssWideKeywords || [
    "initial",
    "inherit",
    "unset",
    "revert",
...
function buildComposablePatterns(
  vscodeData: VSCodeCSSData,
): ComposablePattern[] {
...
function generateUnifiedData(): Effect.Effect<UnifiedData, DataLoadError> {
  return Effect.gen(function* () {
    ...
    const properties = mergeProperties(csstreeData, vscodeData);
...
function writeUnifiedData(data: UnifiedData): Effect.Effect<void, WriteError> {
  return Effect.gen(function* () {
    ...
    try {
      ...
      return yield* Effect.fail(
        new WriteError(outputPath, e instanceof Error ? e.message : "Unknown error"),
      );
    }
  });
}
