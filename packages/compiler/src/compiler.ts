...
const classRegistry = new Map<string, {
...
function generateComponentClassName(name: string, props: CSSProperties): string {
  ...
  const fileName = currentFileScope.split('/').pop()?.replace(/\.(ts|tsx|js|jsx)$/, '') || 'unknown';
...
export function generateCSS(): string {
  ...
  const atomicClasses = Array.from(classRegistry.entries())
    .filter(([key]) => key.startsWith('atomic:'));
  ...
  const componentClasses = Array.from(classRegistry.entries())
    .filter(([key]) => !key.startsWith('atomic:'));
...
export function getRegisteredClasses(): Map<string, {
  properties: CSSProperties;
  className: string;
  fileScope: string;
}> {
