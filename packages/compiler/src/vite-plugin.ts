...
export function cssTSPlugin(options: CSSTSPluginOptions = {}): Plugin {
	...
	return {
		...
		async transform(code, id) {
			...
			const shouldProcess =
				opts.include.some((pattern) => id.match(new RegExp(pattern.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*")))) &&
				!opts.exclude.some((pattern) => id.match(new RegExp(pattern.replace(/\*\*/g, ".*").replace(/\*/g, "[^/]*"))));
...
function extractAndReplaceStyles(code: string, fileId: string): string {
	...
	return code.replace(styleRegex, (match, name, propsStr) => {
		...
		const className = `${fileId
			.split("/")
			.pop()
			?.replace(/\.(ts|tsx)$/, "")}_${name}__${hash}`;
